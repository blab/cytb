...
if __name__ == '__main__':
    ...
    if args.metadata:
        ...
        for name, trait in assignment.items():
            ...
            if name in metadata[id_column].values and 'focal' in metadata.columns:
    ...
    if args.clade_node_data and "clade_membership" in assignment:
        ...
        if args.clade_recency is not None and args.metadata:
            ...
            for strain, info in clades.items():
                if strain in metadata[id_column].values:
                    date_str = metadata.loc[metadata[id_column] == strain, 'date'].values[0]
